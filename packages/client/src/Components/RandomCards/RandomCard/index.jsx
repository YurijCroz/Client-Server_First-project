...
export default function RandomCard(props) {
  ...
  useEffect(()=> {
    ...
    if(buttonClickID===3){
      setInfoText(`Location: ${user.country} ${user.state}`);
    } else 
    if(buttonClickID===4){
      setInfoText(`Date of Birth: ${format(new Date(user.dateOfBirth), 'P')} age: ---`);
    } else {
      setInfoText();
    }
  },[buttonClickID, user]);
  ...
  return (
    <article className={scss.card}>
      <section className={classnames(scss.card__header, user.gender==='male' ? scss.male : scss.female)}>
        <img className={scss.card__header_avatar} src={user.pictureName} alt={`${user.firstName} ${user.lastName}`} />
      </section>
